...
<nav>
	<ul>
        ...
        <li><img src="../../../img/listProfessor.png" class="imgMenu" draggable="false"><p class="legenda" >Listar Professores</p></li>
...
<table>
	...
	while($rowList = mysqli_fetch_array($queryAgendamento)){
		for($i = 0; $i<14;$i++){
		if(substr($rowList['CPF'],$i,1) !='.' && substr($rowList['CPF'],$i,1) !='-'){
			$cpf .= substr($rowList['CPF'],$i,1);
			}
		}
		?>
			<tr id="linha<?php echo $cpf?>">
            		<?php $cpf = "";?>
           	  <td><img class="ft" src="<?php echo $rowList['FOTO']?>" draggable="false"></td>
                <td><?php echo $rowList['NOME']." ".$rowList['SOBRENOME']?></td>
                <td>
				<?php if($rowList['PERMISSAO'] == 0){
						echo "Professor";
					}else{
						echo "Administrador";
						}
				...
				?></td>
                <td><a href="../../../php/recuperarSenha.php?cpf=<?php echo $rowList['CPF'] ?>">Email de recuperação</a></td>
                <td name="update"><a href="../../../php/updateProfessor.php?codigo=<?php echo $rowList['CPF'];?>"><img src="../../../img/edit-file.png" width="30"></a></td>
                <td name="delete"><img src="../../../img/delete.png" width="30" onclick="excluir('<?php echo $rowList['CPF']?>')"></td>
	  </tr>
		<?php
		}
	?>
		...
		$('#menuH').click(function(){
			...
			$('main, header, footer').click(function(){
				...
				$('#print').click(function(){
					...
					tela.document.getElementById('print').style.display = "none";
					tela.print();
					tela.window.close();
					});
    });
